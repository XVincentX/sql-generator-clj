...
(defmethod where-clause :sqlserver [spec fields] (when (:where spec) (str "WHERE " (where-clause* (:where spec) fields))))
(defmethod where-clause :default [spec fields]
  (.trim (str
          (if (some? (:where spec)) (str "WHERE " (where-clause* (:where spec) fields) " ") " ")
          (top-clause spec))))
